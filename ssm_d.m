% ssm_D.m state space matrix D
% x_dot = A * x + B * control 
% y   = C * x + D * control  
function D_reduce = ssm_d( state_eq, state_dot_eq, control_eq, xcg )
...
for j = 1 : no_col
    ...
    for i = 1 : no_row
        tol = 0.1;
        a0 = dy_du( state_eq, control_eq, xcg, row(i), column(j), dv0 );
        a1 = dy_du( state_eq, control_eq, xcg, row(i), column(j), dv1 );
        a2 = dy_du( state_eq, control_eq, xcg, row(i), column(j), dv2 );
        ...
        while k <= 100 
            ...
            if new_dv <= min_dv * state_eq( column(j) )
                ...
                a2 = a1;
                a1 = a0;
                a0 = dy_du( state_eq, control_eq, xcg, row(i), column(j), new_dv );
                b1 = b0;
